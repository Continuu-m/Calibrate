...
async def generate_user_digest(db: Session, user: User):
    ...
    and constructs the HTML for the daily digest email with robust error handling.
    """
    try:
        ...
        print(f"Error generating or sending digest for user {user.id}: {e}")
        # In a real app, you might re-raise or use a better logger
